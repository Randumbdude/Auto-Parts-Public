...
public class CameraEntity extends ClientPlayerEntity {
	...
	public CameraEntity(MinecraftClient mc, ClientWorld world, ClientPlayNetworkHandler nethandler, StatHandler stats,
	...
	private static CameraEntity create(MinecraftClient mc) {
		CameraEntity camera = new CameraEntity(mc, mc.world, mc.player.networkHandler, mc.player.getStatHandler(),
		...
		if (player != null) {
			camera.refreshPositionAndAngles(player.getX(), player.getY(), player.getZ(), player.getYaw(),
					player.getPitch());
