...
public class WorldUtils {
	...
	public static List<BlockPos> getLoadedNetherPortals() {
        ...
        Predicate<RegistryEntry<PointOfInterestType>> predicate = (poi) ->
                poi.getKey().get() == PointOfInterestTypes.NETHER_PORTAL;

        for (ChunkPos chunk : chunks) {
            ArrayList<PointOfInterest> poiList = sw.getPointOfInterestStorage().getInChunk(predicate, chunk,
                    PointOfInterestStorage.OccupationStatus.ANY).collect(Collectors.toCollection(ArrayList::new));
