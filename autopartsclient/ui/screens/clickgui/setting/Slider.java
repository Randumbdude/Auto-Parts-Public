...
public class Slider extends Component{
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		DrawableHelper.fill(matrices, parent.parent.x, parent.parent.y + parent.offset + offset, parent.parent.x + renderWidth,
				parent.parent.y + parent.offset + offset + parent.parent.height, new Color(0, 150, 0).getRGB());
		
		if (sliding) {
			if(diff == 0) {
				numSet.setValue(numSet.getMin());
			} else {
				double newNum = roundToPlace(((diff) * (numSet.getMax() - numSet.getMin()) + numSet.getMin()), 2);
